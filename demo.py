...
def handle_message():
    ...
    if selected_symptoms:
        ...
        try:
            ...
            # Initialize a dictionary to track the match score for each function
            function_scores = {}
            ...
            for token in tokens:
                ...
                for row in all_keywords:
                    ...
                    if token in keyword_list:
                        function_name = row[1]
                        key = row[2]
                        logging.debug(f"Keyword matched: {token}, function_name: {function_name}, key: {key}")
                        
                        # Increment the match score for the corresponding function
                        if function_name in function_scores:
                            function_scores[function_name]['score'] += 1
                        else:
                            function_scores[function_name] = {'score': 1, 'key': key}
            
            logging.debug(f"Function match scores: {function_scores}")

            # Check if there were any matches
            if function_scores:
                # Prioritize the function with the highest score
                ...
                if best_match_function == "predict_future_diabetes":
                    response = predict_future_diabetes(user_msg)
                elif best_match_function == "predict_heart_disease":
                    response = predict_heart_disease(user_msg)
                elif best_match_function == "predict_hypertension":
                    response = predict_hypertension(user_msg)
                elif best_match_function == "get_employee_data":
                    emp_id = extract_checkup_id(user_msg)
                    fields = [field for field in ['age', 'height', 'weight', 'blood_pressure', 'pulse'] if field in user_msg]
                    response = get_employee_data(emp_id, fields)
                elif best_match_function == "count_diabetes_patients":
                    response = count_diabetes_patients(user_msg)
                else:
                    response = handle_keyword(best_match_function, function_scores[best_match_function]['key'])
            else:
                response = "I couldn't find a relevant keyword to process your request."
...
def handle_message():
    ...
    if selected_symptoms:
        ...
        try:
            ...
            # Query the response table for a direct match
            ...
            if result:
                ...
                # Tokenize the user message for further processing
                doc = nlp(user_msg)
                tokens = [token.text for token in doc]
                logging.debug(f"Tokens extracted from user message: {tokens}")
                ...
                for row in all_keywords:
                    ...
                    # Check if all keywords are in the user's message
                ...
                if function_scores:
                    ...
                    response = call_function_based_on_name(best_match_function, user_msg, key_value)  # <-- Pass key_value here
...
# Helper function to call the respective function based on function name
def call_function_based_on_name(function_name, user_msg, key_value):
    ...
    elif function_name == "handle_keyword":
        return handle_keyword(function_name, key_value)
    else:
    ...
    #         # Update the SQL query to fetch both function_name and key
    #         cursor.execute("SELECT function_name, `key` FROM response_table WHERE user_message = %s", (user_msg,))
    #         result = cursor.fetchone()

    #         if result:
    #             function_name, key_value = result  # Unpack both values
    #             logging.debug(f"Function found in response table: {function_name}, key: {key_value}")
    ...
    #                 response = globals()[function_name](function_name, key_value)
    #             else:
    #                 response = "No matching function found."
    #         else:
    #             # Remaining code for token processing...
    ...
    #                 elif best_match_function == "count_diabetes_patients":
    #                     response = count_diabetes_patients(user_msg)
    #                 elif best_match_function in ['handle_disease', 'handle_keyword']:
    #                     response = globals()[best_match_function](best_match_function, key_value)
    #                 else:
    #                     response = "No matching function found."
