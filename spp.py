...
#                 logging.error(f"Missing column in data: {col}")
#                 return "Feature columns missing in data."
...
# Function to predict future diabetes risk
...
def handle_message():
    # Retrieve user message and selected symptoms from the form
    ...
    # If symptoms are selected, predict diagnosis based on them
    if selected_symptoms:
        symptoms = {symptom: 1 for symptom in selected_symptoms}  # Convert symptoms into dictionary
        ...
        try:
            ...
            # Tokenize the user's message for keyword extraction
            doc = nlp(user_msg)
            tokens = [token.text for token in doc]
            logging.debug(f"Tokens extracted from user message: {tokens}")
            keyword_found = False

            # Fetch keywords and function mappings from the database
            ...
            # Loop through tokens and match against the keywords from the database
            for token in tokens:
                logging.debug(f"Processing token: {token}")
                for row in all_keywords:
                    keyword_list = row[0].split(',')  # Handle multiple keywords in the database
                    ...
                    if token in keyword_list:
                        ...
                        # Call the appropriate function based on the keyword match
                        if function_name == "predict_future_diabetes":
                            ...
                            response = predict_heart_disease(user_msg)
                        elif function_name == "predict_hypertension":
                            response = predict_hypertension(user_msg)    
                            ...
                            response = count_diabetes_patients(user_msg)
                        else:
...
#         # Loop through the sentences and match against user message
#         for keyword, function_name, key in all_keywords:
#             if keyword.lower() in user_msg:
#                 logging.debug(f"Sentence matched: {keyword}, function_name: {function_name}, key: {key}")

#                 # Call the appropriate function based on the match
#                 if function_name == "predict_future_diabetes":
#                     response = predict_future_diabetes(user_msg)
#                 elif function_name == "predict_heart_disease":
#                     response = predict_heart_disease(user_msg)
...
def handle_message():
    ...
    if selected_symptoms:
        ...
        try:
            ...
            # Fetch all keywords, function names, and keys from the keyword_functions table
            cursor.execute("SELECT keyword, function_name, `key` FROM keyword_functions")
            all_keywords = cursor.fetchall()

            function_matches = []

            # Loop through all the keyword-function pairs and count matches for each function
            for row in all_keywords:
                keyword_list = row[0].split(',')  # List of keywords for each function
                function_name = row[1]
                key = row[2]

                # Count the number of matched keywords in the user message tokens
                match_count = sum(1 for keyword in keyword_list if keyword in tokens)

                # Store function, key, and number of matched keywords
                if match_count > 0:  # Only consider functions with at least one match
                    function_matches.append((function_name, key, match_count))
                    logging.debug(f"Function '{function_name}' matches {match_count} keyword(s)")

            if function_matches:
                # Sort the matches by the number of matched keywords in descending order
                # The function with the most matched keywords will come first
                function_matches.sort(key=lambda x: x[2], reverse=True)
                best_match_function, key_value, max_matches = function_matches[0]

                logging.debug(f"Best match function: {best_match_function} with {max_matches} matched keyword(s), key: {key_value}")

                # Call the corresponding function based on the best match
                response = globals()[best_match_function](user_msg, key_value)

                # Insert the matched function into the response table for future queries
